...
db = sqlite3.connect('/home/tikrong/mysite/english.db')
...
def exercise():
    ...
    if request.args.get("exercise_id"):
        ...
        cursor = db.execute("select tags from exercises where id == :tmp", {"tmp": exercise_id})
        tag = cursor.fetchall()
        tag = tag[0]["tags"]

        cursor = db.execute("select quantity from exercises where id == :quantity", {"quantity": exercise_id})
        quantity = cursor.fetchall()
        quantity = quantity[0]["quantity"]

        #selecting random questions with corresponding tag from questions table
        cursor = db.execute("""SELECT * FROM questions WHERE id IN (SELECT id FROM questions
        ...
        cursor = db.execute("SELECT name FROM exercises WHERE id = :cur_id", {"cur_id": exercise_id})
        session["exercise_name"] = cursor.fetchall()
        session["exercise_name"] = session["exercise_name"][0]["name"]
        cursor = db.execute("SELECT description FROM exercises WHERE id = :cur_id", {"cur_id": exercise_id})
        session["description"] = cursor.fetchall()
        session["description"] = session["description"][0]["description"]
        return redirect("/exercise_blank")

    else:

        cursor = db.execute("select distinct topic from exercises") # getting list with all topics
        topics = cursor.fetchall()
        exercises = dict()
        for cur_topic in topics:
            cursor = db.execute(""" SELECT exercises.id, name, type, quantity, correct_answers
...
def score():
    cursor = db.execute(""" SELECT * FROM exercises LEFT JOIN scores ON exercises.id = scores.exercise_id
                        AND scores.username = :user""", {"user": session["username"]})
    scores = cursor.fetchall()

    cursor = db.execute("select distinct topic from exercises")
    topics = cursor.fetchall()

    for i in range(len(topics)):
        # calcutale maximum points for this topic
        cursor = db.execute("SELECT SUM(quantity) FROM exercises WHERE topic = :topic",
                                            {"topic": topics[i]['topic']})
        topics[i]['max_points'] = cursor.fetchall()[0]['SUM(quantity)']



        # calculate user's points
        cursor = db.execute(""" SELECT SUM(correct_answers) FROM exercises LEFT JOIN scores ON exercises.id = scores.exercise_id
                                     AND scores.username = :user AND topic = :topic""",
                                     {"user": session["username"], "topic": topics[i]["topic"]})
        topics[i]['points'] = cursor.fetchall()[0]['SUM(correct_answers)']


        # calculate possible points for the exercises user has already done
        cursor = db.execute("""SELECT SUM(quantity) FROM exercises LEFT JOIN scores ON exercises.id = scores.exercise_id
                                    AND scores.username = :user WHERE topic = :topic AND correct_answers IS NOT NULL """,
                                    {"user": session["username"], "topic": topics[i]["topic"]})
        possible_points = cursor.fetchall()[0]['SUM(quantity)']


        # calculate the number of exercises done
        cursor = db.execute("""SELECT COUNT(*) FROM exercises LEFT JOIN scores ON exercises.id = scores.exercise_id
                                    AND scores.username = :user WHERE topic = :topic AND correct_answers IS NOT NULL """,
                                    {"user": session["username"], "topic": topics[i]["topic"]})
        topics[i]["exercises_done"] = cursor.fetchall()[0]['COUNT(*)']


        cursor = db.execute("SELECT COUNT(*) FROM exercises WHERE topic = :topic",
                                            {"topic": topics[i]['topic']})
        topics[i]["exercises_total"] = cursor.fetchall()[0]['COUNT(*)']
        ...
        cursor = db.execute("""SELECT name, correct_answers, quantity, date, exercises.id FROM exercises LEFT JOIN scores ON exercises.id = scores.exercise_id
                                   AND scores.username = :user WHERE topic = :topic""",
                                  {"user": session["username"], "topic": topics[i]['topic']})
        topics[i]["exercises"] = cursor.fetchall()
...
def exercise_blanks():
    ...
    # problem of this method is that I can enter html tags and they are working!!!
        ...
        if not session["answers"]:
            cursor = db.execute("SELECT * FROM answers WHERE id_question = :number",
                                {"number": session["questions"][session["question_n"]]["id"]})
            session["answers"] = cursor.fetchall()
        ...
        if session["answer_n"] == len(session["answers"]):
            ...
            # if the exercise is over
                ...
                session["score"] += session["correct"]/(session["answer_n"])
                cursor = db.execute("SELECT * FROM exercises WHERE id = :cur_id", {"cur_id": session["exercise_id"]})
                tmp = cursor.fetchall()[0]

                cursor = db.execute("SELECT * FROM scores WHERE username = :user AND exercise_id = :cur_id",
...
@app.route("/learn")
def learn():
    cursor = db.execute("SELECT DISTINCT topic FROM learn")
    topics = cursor.fetchall()

    exercises = dict()

    for topic in topics:
        cursor = db.execute("SELECT name FROM learn WHERE topic = :topic", {"topic": topic["topic"]})
        exercises[topic["topic"]] = cursor.fetchall()
...
def learn_smth(name):
    ...
    cursor = db.execute("SELECT topic, content FROM learn WHERE name = :name", {"name": name})
...
def users():
    ...
    if request.method == "GET":
        ...
        if request.form.get("password_change"):
            ...
            cursor = db.execute("SELECT * FROM users WHERE username = :user", {"user": session["user"][0]["username"]})
            session["user"] = cursor.fetchall()
            cursor = db.execute("SELECT * FROM users")
            users = cursor.fetchall()
            ...
            cursor = db.execute("SELECT * FROM users WHERE username = :user", {"user": request.form.get("username_change")})
            session["user"] = cursor.fetchall()
            cursor = db.execute("SELECT * FROM users")
            users = cursor.fetchall()
            ...
            cursor = db.execute("SELECT * FROM users WHERE username = :user", {"user": request.form.get("username")})
            session["user"] = cursor.fetchall()
            cursor = db.execute("SELECT * FROM users")
            users = cursor.fetchall()
...
def feedback():
    if request.method == ["GET"]:
        cursor = db.execute("SELECT * FROM feedback WHERE status is null")
